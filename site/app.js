...
function renderMatches() {
  const grouped = groupBy(
    data.matches || [],
    m => `${m.datum_iso || ''}||${m.datum || 'Onbekende datum'}||${m.fase || 'Wedstrijd'}`
  );

  els.matchesContainer.innerHTML = Object.entries(grouped)
    .map(([groupKey, groupMatches]) => {
      const [isoDate, dateLabel, phaseLabel] = groupKey.split('||');

      return `
        <section class="group" data-date="${e(isoDate)}">
    ...
    <article class="card">
      <div class="card-shell">
        <div class="bar blue match-bar">
          ...
          <div class="match-item match-result">${e(result)}</div>
...
function renderBonusCard(question) {
  ...
  return `
    <article class="card">
      <div class="card-shell">
        <div class="bar blue bonus-question-bar">
          Bonusvraag ${e(String(question.nr || ''))}: ${e(question.vraag || '')}
        </div>
        <div class="correct-answer">
          Correct antwoord: <strong>${e(question.correct_antwoord || 'nog niet ingevuld')}</strong>
        </div>
        <table class="data-table">
          <thead>
            <tr>
...
function scrollToToday() {
  ...
  const groups = [...document.querySelectorAll('.group[data-date]')];
  ...
  // fallback: als alles in het verleden ligt, scroll naar laatste wedstrijddag
